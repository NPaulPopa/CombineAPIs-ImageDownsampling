...
extension PhotoAPIManager {
    ...
    func downsampledImagesPublisher(urlPublishers: [AnyPublisher<CGImageSource, Error>]) async -> AnyPublisher<UIImage, Never> {
        return try! await withThrowingTaskGroup(of: AnyPublisher<UIImage, Never>.self) { group in
            ...
            for try await image in group {
                imagePublisher = image.eraseToAnyPublisher()
            }
